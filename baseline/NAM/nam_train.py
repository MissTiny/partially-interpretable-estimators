...
def _create_computation_graph(
    ...
    y_validation, x_real,y_real,batch_size
):
...
def _update_metrics_and_checkpoints(sess,
  ...
  if not FLAGS.regression:
      validation_acc = metric_scores['acc'](sess)
      real_acc = metric_scores['real_acc'](sess)
  real_metric = metric_scores['real_metric'](sess)
  if FLAGS.debug:
  ...
  return curr_best_epoch, best_validation_metric, best_train_metric,best_validation_acc,best_real_acc,best_real_metric


def training(x_train, y_train, x_validation,
             y_validation,x_real,y_real,
             logdir):
  ...
  if FLAGS.regression:
    ...
    best_real_metric = np.inf * np.ones(FLAGS.n_models) #new
  else:
    best_train_metric = np.zeros(FLAGS.n_models)
    best_validation_metric = np.zeros(FLAGS.n_models)
    best_validation_acc = np.zeros(FLAGS.n_models) #new
    best_real_acc = np.zeros(FLAGS.n_models) #new
    best_real_metric = np.zeros(FLAGS.n_models) #new
  ...
  with tf.Graph().as_default():
    ...
    graph_tensors_and_ops, metric_scores = _create_computation_graph(
        x_train, y_train, x_validation, y_validation, x_real,y_real,batch_size)
        ...
        for n in range(FLAGS.n_models):
          ...
          if epoch % FLAGS.save_checkpoint_every_n_epochs == 0:
              if FLAGS.regression:
                 ...
                 best_train_metric[n],_,_,best_real_metric[n]) = _update_metrics_and_checkpoints(
                     sess, epoch, metric_scores[n], curr_best_epoch[n],
                     best_validation_metric[n], best_train_metric[n],None,None,best_real_metric[n], model_dirs[n],
                     best_checkpoint_dirs[n], metric_name)
              else:
                (curr_best_epoch[n], best_validation_metric[n],
                 best_train_metric[n],best_validation_acc[n],best_real_acc[n],best_real_metric[n]) = _update_metrics_and_checkpoints(
                     sess, epoch, metric_scores[n], curr_best_epoch[n],
                     best_validation_metric[n], best_train_metric[n],best_validation_acc[n],best_real_acc[n],best_real_metric[n], model_dirs[n],
                     best_checkpoint_dirs[n], metric_name)
  ...
  for n in range(FLAGS.n_models):
      if FLAGS.regression:
          tf.logging.info(
        'Model %d: Best Epoch %d, Individual %s: Train %.4f, Validation %.4f, Test: %.4f',
        n, curr_best_epoch[n], metric_name, best_train_metric[n],
        best_validation_metric[n],best_real_metric[n])
          return saver_hooks,graph_tensors_and_ops, np.mean(best_train_metric), np.mean(best_validation_metric),np.mean(best_real_metric),[metric['train_op']for metric in metric_scores],[metric['train_op']for metric in metric_scores]
      else:
          tf.logging.info(
        'Model %d: Best Epoch %d, Individual %s: Train %.4f, Validation %.4f, Validation acc: %.4f, Test ACC: %.4f, Test ROC: %.4f',
        n, curr_best_epoch[n], metric_name, best_train_metric[n],
        best_validation_metric[n],best_validation_acc[n],best_real_acc[n],best_real_metric[n])
          return saver_hooks,graph_tensors_and_ops, np.mean(best_train_metric), np.mean(best_validation_metric), np.mean(best_validation_acc),np.mean(best_real_acc),np.mean(best_real_metric),[metric['train_op']for metric in metric_scores],[metric['train_op']for metric in metric_scores]

  
#added new function
def predict(model, x,y_test):
     metric, acc = graph_builder.graph_pred(x,y_test,model,regression=False)
     return metric, acc
#################
...
def single_split_training(data_gen,x_real,y_real,
                          logdir):
  ...
  curr_logdir = os.path.join(logdir, 'fold_{}',
                             'split_{}').format(FLAGS.fold_num,
                                                FLAGS.data_split)
  return training(x_train, y_train, x_validation, y_validation,x_real,y_real, curr_logdir)
