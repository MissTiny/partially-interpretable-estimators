...
'''
#newly added
def acc_score_classification(sess,y_true,pred_tensor,dataset_init_op):
    y_pred = generate_predictions(pred_tensor, dataset_init_op, sess)
    
    return sk_metrics.accuracy_score(y_true,[x.round() for x in y_pred],normalize=True)
...
####new
def rrmse_loss(sess, y_true, pred_tensor,
              dataset_init_op):
  y_pred = generate_predictions(pred_tensor, dataset_init_op, sess)
  return rrmse(y_true, y_pred)
...
def rrmse(y_true, y_pred):
  ...
  for i in range(len(y_true)):
     top.append(float(math.pow(float(y_true[i]-y_pred[i]),2)))
     bottom.append(float(math.pow(float(y_true[i]-average),2)))
...
#####added new
'''
def graph_pred(x,y_test,nn_model,regression=False):
    predictions = nn_model(x, training=False)
    ...
    test_metric = functools.partial(
      ...
      y_true=y_test,
      pred_tensor=y_pred,
      dataset_init_op=None)
    if regression:
      test_acc = functools.partial(
          acc_score_regression,
          y_true=y_test,
          pred_tensor=predictions,
          dataset_init_op=None)
     else:
      test_acc = functools.partial(
          acc_score_classification,
          y_true=y_test,
          pred_tensor=y_pred,
          dataset_init_op=None)
    return test_metric, test_acc
'''
###########################
def build_graph(
    x_train,
  ...
  x_batch, (train_init_op, test_init_op,real_init_op) = create_iterators((x_train, x_test,x_real),
                                                            batch_size)
  ...
  # Use RMSE for regression and ROC AUC for classification.
  evaluation_metric = rrmse_loss if regression else roc_auc_score
  acc_score = acc_score_classification
  ...
  if not regression:
      test_acc = functools.partial(
          ...
          pred_tensor=y_pred,
          dataset_init_op=test_init_op)
      real_acc = functools.partial(
          acc_score,
          y_true=y_real,
          pred_tensor=y_pred,
          dataset_init_op=real_init_op)
  real_metric= functools.partial(
      evaluation_metric,
      y_true=y_real,
      pred_tensor=y_pred,
      dataset_init_op=real_init_op)
  ...
  if regression:
      eval_metric_scores = {'test': test_metric, 'train': train_metric,'real_metric':real_metric,'train_op': train_init_op,'test_op': test_init_op}
  else:
    eval_metric_scores = {'test': test_metric, 'train': train_metric,'acc': test_acc,'real_acc':real_acc,'real_metric':real_metric,'train_op': train_init_op,'test_op': test_init_op}
